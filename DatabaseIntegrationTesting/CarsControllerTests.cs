...
{
    public class CarsControllerTests : IClassFixture<CustomWebApplicationFactory<MiniCarRentalAPI.Program>>
    {
        private readonly ITestOutputHelper _testOutputHelper;
        private readonly CustomWebApplicationFactory<MiniCarRentalAPI.Program> _factory;
        private readonly IConfigurationRoot _config;

        public CarsControllerTests(ITestOutputHelper testOutputHelper, CustomWebApplicationFactory<MiniCarRentalAPI.Program> factory)
        {
        ...
        [Theory]
        [InlineData("/api/Cars/1000")]
        public async Task GetCarWrongIndexTest(string url)
        {
            // Arrange
            using var db = _factory.CreateThrowawayDb();
            var client = _factory.CreateClientForDatabase(db);
